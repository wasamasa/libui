...
(define (handler-draw area-handler area params)
  (let ((path #f)
        ...
        (context (ui:area-draw-params-context params))
        (area-width (ui:area-draw-params-area-width params))
        (area-height (ui:area-draw-params-area-height params))
        (graph-width -1)
        (graph-height -1))
...
(define (handler-mouse-event area-handler area event)
  (let*-values (((graph-width graph-height)
                 (graph-size (ui:area-mouse-event-area-width event)
                             (ui:area-mouse-event-area-height event)))
                ((xs ys) (point-locations graph-width graph-height)))
    (let loop ((i 0))
      (if (< i 10)
          (if (in-point? (ui:area-mouse-event-x event) (ui:area-mouse-event-y event)
